...
export type route_handler = (req: IncomingMessage, res: ServerResponse, ctx: handler_context) => Promise<any>;
...
export function Send(status: number) {
    return function (target: any, name: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = async function (req: IncomingMessage, res: ServerResponse, ctx: handler_context) {
...
export function Pipe(target: any, name: string, descriptor: PropertyDescriptor) {
    const original = descriptor.value;

    descriptor.value = async function (req: IncomingMessage, res: ServerResponse, ctx: handler_context) {
        const result_read_stream: ReadStream = await original.call(this, req, res, ctx);
...
export function ParseBody(target: any, name: string, descriptor: PropertyDescriptor) {
    const original = descriptor.value;

    descriptor.value = async function (req: IncomingMessage, res: ServerResponse, ctx: handler_context) {

        req.body = await json(req, {limit: '5mb'});
...
export function Catch(target: any, name: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (req: IncomingMessage, res: ServerResponse, ctx: handler_context) {
        try {
            ...
            return send(res, err.statusCode || 500, new Response({ error_message: err.message }))
        }
    };
...
export function Validate(schema: ValidationSchema) {
    ...
    return function (target: any, name: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = async function (req: IncomingMessage, res: ServerResponse, ctx: handler_context) {
            ...
            const validationResult: ValidationError[] | boolean = check({ query, body, params });
...
export type Decoder = (token: string) => object
...
export function DecodeAccessTokenToUserData(decoder: Decoder) {
    return function (target: any, name: string, descriptor: PropertyDescriptor) {
...
export function AccessToPublishedItem (decoder: Decoder) {
    return function (target: any, name: string, descriptor: PropertyDescriptor) {
...
export function AcceptACL(role: string | string[]) {
    ...
    return function (target: any, name: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = async function (req: IncomingMessage, res: ServerResponse, ctx: handler_context & { role: string }) {
...
export function SetHeader(header_name: string, header_value: number | string | string[]) {
    return function (target: any, name: string, descriptor: PropertyDescriptor) {
        const original = descriptor.value;
