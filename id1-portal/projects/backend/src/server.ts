...
export const server = micro(
  cors()(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === "OPTIONS") {
      const corsWhitelist = [
        CONFIGURATIONS.SERVER.ADMIN_HOST,
        CONFIGURATIONS.SERVER.PUBLIC_HOST,
      ];

      let headersOrigin = (CONFIGURATIONS.SERVER.NODE_ENV = "development"
        ? "*"
        : req.headers.origin || "");

      corsWhitelist.indexOf(headersOrigin) !== -1;
      ...
      res.setHeader("Access-Control-Allow-Headers", [
        "content-type,x-prerender,authorization",
      ]);
    ...
    if (pluginsRegex.test(req.url!)) {
      ...
      const pluginRoutesPath = path.resolve(
        pluginsPath,
        `${pluginName}/routes`
      );
      ...
      if (!(await fileIsExist(pluginRoutesPath))) {
        return send(
          ...
          404,
          new Response({
            error_message: EXCEPTION_MESSAGES.ON_ROUTE_NOT_FOUND_EXCEPTION,
          })
        );
      }
    ...
    if (matched)
      return await matched(req, res).catch((err: Error) => {
        ...
        return send(
          ...
          418,
          new Response({
            error_message: EXCEPTION_MESSAGES.ON_UNHANDLED_ERROR_EXCEPTION,
          })
        );
      });
    return send(
...
process.on(
  "unhandledRejection",
  (error: {} | null | undefined, promise: Promise<any>) => {
...
setInterval(() => {
  ...
  const to_mb = (key: string, value: number) =>
