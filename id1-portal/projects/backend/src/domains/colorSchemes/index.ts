...
export class color_schemes_with_validation {
    ...
    static async update_color_scheme(data: update_color_scheme_payload) {
        await Users.checkIfUserHavePermissionToAccess(parseInt(data.ctx.userId!), Entities, data.ctx.role!);
        if (await color_schemes.isDefaultColorScheme(data.id))throw new Error(`It is forbidden to update the default color scheme.`);
    ...
    static async delete_color_scheme_by_id(data: delete_color_scheme_by_id_payload) {
        await Users.checkIfUserHavePermissionToAccess(parseInt(data.ctx.userId!), Entities, data.ctx.role!);
        if (await color_schemes.isDefaultColorScheme(data.id)) throw new Error(`It is forbidden to delete the default color scheme.`);
